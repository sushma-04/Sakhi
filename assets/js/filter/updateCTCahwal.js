const updateCTCSakhiReport = async (reportId) => {
    const response = await fetch(
      `https://drdajalgaon.com/sakhi-api/public/CTCAhwalQuestions/showbyid/${reportId}`
    );
      ...
      document.getElementById("remark1_div").style.display="none";
        ...
        const questionHTML = question_data_with_selected_options .map((question_data_with_selected_options, i) => {
            ...
            <label class="form-label">${i + 1}. ${
              question_data_with_selected_options.CTC_questions
        }</label>
            <div class="form-check d-flex justify-content-between">
            <div>
           ${question_data_with_selected_options.answers}
          </div>
              <p style="color: gray; font-size: 14px;    padding-right: 30px;">दर : ${
                question_data_with_selected_options.rate
              }</p>
            </div>
          </div>`
          })
          ...
          submitBtn.addEventListener("click", async () => {
            ...
            try {
              const response = await evaluationApi.CTCSakhiAhwalupdate(report.id, updatedReportData);
