const updateKrushiSakhiReport = async (reportId) => {
    const response = await fetch(
      `https://drdajalgaon.com/sakhi-api/public/krushiMandhanQuestion/showbyid/${reportId}`
    );
      ...
      document.getElementById("remark1_div").style.display="none";
        ...
        const questionHTML = question_data_with_selected_options .map((question_data_with_selected_options, i) => {
            ...
            <label class="form-label">${i + 1}. ${
              question_data_with_selected_options.krushi_questions
        }</label>
           ...
           ${question_data_with_selected_options.selected_option===1?"होय":"नाही"}
              ...
              <p style="color: gray; font-size: 14px;    padding-right: 30px;">दर : ${
                question_data_with_selected_options.krushi_rate
              }</p>
            </div>
          </div>`
          })
          ...
          submitBtn.addEventListener("click", async () => {
            ...
            try {
              const response = await evaluationApi.KrushiSakhiAhwalupdate(report.id, updatedReportData);
