const updateBankSakhiReport = async (reportId) => {
    const response = await fetch(
      `https://drdajalgaon.com/sakhi-api/public/bankSakhiQuestions/showbyid/${reportId}`
    );
      ...
      document.getElementById("remark1_div").style.display="none";
        ...
        const questionHTML = question_data_with_selected_options .map((question_data_with_selected_options, i) => {
            ...
            <label class="form-label">${i + 1}. ${
              question_data_with_selected_options.bank_Sakhi_questions
        }</label>
          ...
          submitBtn.addEventListener("click", async () => {
            ...
            try {
              const response = await evaluationApi.BankSakhiAhwalupdate(report.id, updatedReportData);
